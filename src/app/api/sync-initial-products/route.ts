...
export async function GET() {
  try {
    ...
    // Estadísticas por categoría - CONTAR PRODUCTOS ÚNICOS, NO SKUS
    ...
    if (productos) {
      // Agrupar productos únicos por categoría
      const productosUnicosPorCategoria: Record<string, Set<string>> = {};
      productos.forEach((p: any) => {
        if (!productosUnicosPorCategoria[p.categoria]) {
          productosUnicosPorCategoria[p.categoria] = new Set();
        }
        productosUnicosPorCategoria[p.categoria].add(p.nombre);
      });
      
      // Contar productos únicos por categoría
      Object.entries(productosUnicosPorCategoria).forEach(([categoria, nombresUnicos]) => {
        categoriasCount[categoria] = nombresUnicos.size;
      });
    }

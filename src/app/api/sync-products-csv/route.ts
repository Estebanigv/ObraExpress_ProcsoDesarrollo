...
async function procesarPestaña(sheetName: string) {
  ...
  try {
    ...
    const variantes = dataRows.map((row, index) => {
      ...
      // Definir colorRaw antes de usarlo
  ...
  try {
    ...
    if (supabaseAdmin) {
      try {
        ...
        // PASO 2: Obtener datos anteriores desde Supabase para detectar cambios y preservar configuraciones
        console.log('🔍 Obteniendo datos anteriores para detección de cambios...');
        const { data: datosAnteriores, error: datosError } = await supabaseAdmin
          .from('productos')
          .select('codigo, precio_con_iva, disponible_en_web, tiene_imagen');
        
        const mapaPreciosAnteriores = new Map();
        const mapaVisibilidadAnterior = new Map();
        const mapaImagenesAnterior = new Map();
        
        if (!datosError && datosAnteriores) {
          datosAnteriores.forEach(p => {
            mapaPreciosAnteriores.set(p.codigo, p.precio_con_iva);
            // Preservar el estado de visibilidad configurado por el usuario
            mapaVisibilidadAnterior.set(p.codigo, p.disponible_en_web);
            // Preservar el estado de imagen
            mapaImagenesAnterior.set(p.codigo, p.tiene_imagen);
          });
        }
        ...
        const productosParaSupabase = todasLasVariantes.map(v => {
          ...
          return {
            ...
            // Preservar el estado de visibilidad configurado por el usuario
            // Solo usar validacion.isValid si el producto es nuevo (no existe en la BD)
            disponible_en_web: mapaVisibilidadAnterior.has(v.codigo) 
              ? mapaVisibilidadAnterior.get(v.codigo) 
              : false, // Por defecto, productos nuevos empiezan ocultos
            tiene_sku_valido: v.codigo && v.codigo !== `${v.pestaña_origen}-`,
            tiene_stock_minimo: validacion.cumpleStockMinimo,
            // Preservar el estado de imagen si ya existe
            tiene_imagen: mapaImagenesAnterior.get(v.codigo) || validacion.tieneImagenValida,

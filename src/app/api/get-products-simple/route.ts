...
export async function GET() {
  try {
    ...
    console.log('📊 Resultado Supabase:', { 
      ...
      hasData: !!data,
      categorias: data ? [...new Set(data.map((p: any) => p.categoria))].sort() : [],
      primeros3: data?.slice(0, 3).map((p: any) => ({ codigo: p.codigo, categoria: p.categoria, nombre: p.nombre })) || []
    });
    ...
    if (error) {
      ...
      if (data && data.length > 0) {
        ...
        data.forEach((producto) => {
          ...
          // ADMIN VE TODOS LOS PRODUCTOS - NO FILTRAR POR VISIBILIDAD
          // El filtrado de visibilidad es solo para el frontend público
        ...
        // Procesar categorías en el orden correcto - ADMIN VE TODAS
        categoriasOrdenadas.forEach(categoria => {
          if (productosPorCategoria[categoria.name]) {
        ...
        Object.keys(productosPorCategoria).forEach(categoria => {
          if (!productosPorCategoriaOrdenados[categoria]) {

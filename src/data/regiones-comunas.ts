...
export function getComunasByRegion(regionCodigo: string): Comuna[] {
  const region = regionesComunas.find(r => r.codigo === regionCodigo);
  return region ? region.comunas : [];
}

export function getRegionByName(regionNombre: string): Region | undefined {
  return regionesComunas.find(r => r.nombre === regionNombre);
}